...
namespace DiceOrbit.Data.Tile
{
    ...
    public class SampleTileAttribute : TileAttribute
    {
        ...
        public void Activate(Character target)
        {
            ...
            // 피해를 주는 로직 예시 (CombatPipeline 사용)
            var context = new CombatContext(
                null, // 타일(환경) 피해이므로 공격 주체(Source)가 null일 수 있음
                target,
                new CombatAction("샘플 함정 발동", ActionType.Attack, Value) // Value (데미지 수치) 전달
            );
            
            // 파이프라인에 피해 처리를 등록
            CombatPipeline.Instance?.Process(context);

            Debug.Log($"[SampleTileAttribute] {target.name} 유닛이 타일을 밟아 {Value} 만큼의 효과를 받았습니다!");

            // 일회성 함정이라면 발동 후 자신을 삭제 처리
        ...
        public override string GetDescription()
        {
            ...
            return $"지나가거나 턴 종료 시 {Value} 효과, 지속 {durationText}";
        }
    }
}
